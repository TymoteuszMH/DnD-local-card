        ...
        <div class="flex flex-col justify-center gap-5 mb-3!">
            ...
            <div class="relative flex flex-col rounded-md items-center h-16 md:h-22 overflow-visible min-h-[50px]">
                <div class="absolute z-1 flex flex-row gap-2 justify-center h-4 md:h-auto">
                    ...
                    <el-input type="number" input-style="border-radius: inherit; height: 100%; width: 100%; text-align: center; font-size: inherit;" :model-value="card.healthInfo.maxHealthPoints" @input="(value: string) => card.healthInfo.maxHealthPoints = checkMinMaxNum(card.healthInfo.healthPoints, value, 0)"/>
                </div>
            ...
            <div class="relative flex flex-col rounded-md items-center h-16 md:h-22 overflow-visible min-h-[50px]">
                <div class="absolute z-1 flex flex-row gap-2 justify-center h-4 md:h-auto">
                    <LabelField class="text-end! w-min!">Max:</LabelField>
                    <el-input input-style="border-radius: inherit; height: 100%; width: 100%; text-align: center; font-size: inherit;" :model-value="card.healthInfo.maxHitDices" @input="(value: string) => card.healthInfo.maxHitDices = value"/>
                </div>
                <el-input type="number" class="h-full text-2xl md:text-4xl! text-center overflow-hidden" input-style="border-radius: inherit; height: 100%; width: 100%; text-align: center; font-size: inherit" :model-value="card.healthInfo.usedhitDices" @input="(value: string) => card.healthInfo.usedhitDices = checkMinMaxNum(card.healthInfo.usedhitDices, value, 0)"/>
                <div class="absolute text-xs md:text-base bg-black text-white -bottom-[15%] z-1 rounded-lg border w-full h-6 flex items-center justify-center ">Hit Dices</div>
            </div>
        </div>
    </div>
</template>
